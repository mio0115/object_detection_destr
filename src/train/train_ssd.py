    ...
    for idx in range(args.epochs):
        ...
        with torch.no_grad():
            for vdata in valid_loader:
                ...
                running_vloss_class += vloss_model[
                    "class"
                ].item() * vinputs.size(0)
                running_vloss_ciou += vloss_model[
                    "ciou"
                ].item() * vinputs.size(0)
                ...
                if g_vstep % log_interval == 0:
                    ...
                    avg_vloss_cls = (
                        running_vloss_class - prefix_vloss_class
                    ) / (log_interval * args.batch_size)
                    avg_vloss_ciou = (
                        running_vloss_ciou - prefix_vloss_ciou
                    ) / (log_interval * args.batch_size)

                    writer.add_scalar(
                        "Loss/valid/model", avg_vloss_model, g_vstep
                    )
                    writer.add_scalar("Loss/valid/det", avg_vloss_det, g_vstep)
                    writer.add_scalar(
                        "Loss/valid/class", avg_vloss_cls, g_vstep
                    )
                    writer.add_scalar(
                        "Loss/valid/ciou", avg_vloss_ciou, g_vstep
                    )
...
def train_one_epoch(
    ...
    for data in dataloader:
        ...
        if g_step % log_interval == 0:
            ...
            prefix_loss_det, prefix_loss_model = (
                running_loss_det,
                running_loss_model,
            )

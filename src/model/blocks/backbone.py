...
class BackboneBase(nn.Module):
    ...
    def forward_mask(self, x, ori_mask):
        mask = F.interpolate(ori_mask[None].float(), size=x.shape[-2:]).to(torch.bool)[
            0
        ]
        return mask
...
class Joiner(nn.Sequential):
    ...
    def pos_embed(self, tensor, ori_mask):
        mask = self[0].forward_mask(tensor, ori_mask)
        return self[1].pos_embed_forward(tensor, mask).to(tensor.dtype), mask
